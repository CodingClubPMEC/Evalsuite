...
def create_mark(
    *, session: Annotated[Session, Depends(get_session)], mark: Marking
) -> Marking:
...
def read_mark(
    *, session: Annotated[Session, Depends(get_session)], mark_id: UUID
) -> Marking:
...
@marking_router.get("/marking/{event_id}", response_model=Sequence[Marking])
def read_marks_by_event(
    *, session: Annotated[Session, Depends(get_session)], event_id: UUID
) -> Sequence[Marking]:
    marks = session.exec(select(Marking).where(Marking.event_id == event_id)).all()
    return marks
...
def update_mark(
    *,
    ...
    if mark.jury_id != jury_id:
        raise HTTPException(
            status_code=403, detail="Only the jury who created this mark can update it"
        )
...
def delete_mark(
    *, session: Annotated[Session, Depends(get_session)], mark_id: UUID, jury_id: UUID
    ...
    if mark.jury_id != jury_id:
        raise HTTPException(
            status_code=403, detail="Only the jury who created this mark can delete it"
        )
